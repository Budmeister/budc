...
fn compile_bitand(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
...
fn compile_bitor(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest_live = fenv.place_to_addr_mode(dest.clone(), instrs, Proxy2)?;
    let (src, _) = extend_efficient(src, size_src, size_dest, instrs, fenv, Proxy1)?;
    let (dest_dead, live) = extend_efficient(
...
fn compile_equal(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
...
fn compile_noteq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_greater(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_grtreq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_less(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_lesseq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<Instruction<Valid>>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
