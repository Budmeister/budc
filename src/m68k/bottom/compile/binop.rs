...
pub fn compile_binop_iinstr(src: Place, b: BudBinop, dest: Place, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
    ...
    match b {
        ...
        BudBinop::Equal => compile_equal(src, dest, size_src, size_dest, instrs, fenv),
        BudBinop::NotEq => compile_noteq(src, dest, size_src, size_dest, instrs, fenv),
        BudBinop::Greater => compile_greater(src, dest, size_src, size_dest, instrs, fenv),
        BudBinop::GrtrEq => compile_grtreq(src, dest, size_src, size_dest, instrs, fenv),
        BudBinop::Less => compile_less(src, dest, size_src, size_dest, instrs, fenv),
        BudBinop::LessEq => compile_lesseq(src, dest, size_src, size_dest, instrs, fenv),
    }
}

fn compile_plus(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
...
fn compile_minus(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
    // Plus needs a dreg, and it needs to be extended anyway
    let (src, _) = fenv.place_to_dreg(src, instrs, env, Proxy1)?;
    extend(src, size_src, size_dest, instrs)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
...
fn compile_times(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
...
fn compile_div(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
...
fn compile_and(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
...
fn compile_or(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
...
fn compile_bitand(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
...
fn compile_bitor(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, _env: &Environment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest_live = fenv.place_to_addr_mode(dest.clone(), instrs, Proxy2)?;
    let (src, _) = extend_efficient(src, size_src, size_dest, instrs, fenv, Proxy1)?;
    let (dest_dead, live) = extend_efficient(
...
fn compile_bitxor(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment, env: &Environment) -> Result<(), String> {
...
fn compile_equal(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
...
fn compile_noteq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_greater(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_grtreq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_less(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
...
fn compile_lesseq(src: Place, dest: Place, size_src: DataSize, size_dest: DataSize, instrs: &mut Vec<ValidInstruction>, fenv: &mut FunctionEnvironment) -> Result<(), String> {
    let src = fenv.place_to_addr_mode(src, instrs, Proxy1)?;
    let dest = fenv.place_to_addr_mode(dest, instrs, Proxy2)?;
    cmp(src, dest.clone(), size_src, size_dest, instrs, fenv)?;
    let size = max(size_src, size_dest);
